...
> 提示：邮箱请使用你 GitHub 注册的邮箱，否则贡献不会被计入。
>
> 提示：本教程的命令都以主分支 `main` 为例；如果 `git branch` 显示的是 `master`（老版本默认），把命令里的 `main` 换成 `master` 即可。
...
> 注意：`.gitignore` 只对**还没提交过**的文件生效。如果某个文件已经被提交了才想起来要忽略，执行 `git rm --cached 文件名`（把文件移出 Git 跟踪、但保留在电脑上），再提交一次即可。
...
> 提示：如果过几天 PR 提示“冲突”，说明原项目又更新了，而你的 `fix-xxx` 分支还是旧代码。此时切回 `fix-xxx` 分支，把上游最新代码合并进来，再重新推送即可（怎么解决冲突见下一节）：
> ```bash
...
```
> 注意：只对“已被 Git 跟踪”的文件生效，新建的文件不会被删除。改动一旦丢弃就找不回来了，动手前确认一下。
...
```
> 注意：只适合**还没 push** 的提交；已经 push 的就别再 amend 了（会改写历史，别人同步时会很痛苦），重新提交一条新的即可。
...
```
> 注意：`--hard` 很危险，执行前确认没有需要保留的修改。
> 提示：被丢掉的提交几天内还能找回来——先 `git reflog` 找到它，再 `git reset --hard 那个提交ID` 即可。
...
```
> 提示：默认只收起“已跟踪文件”的修改；新建的文件想一起收起，用 `git stash -u`。
...
> 提示：如果冲突太多、改乱了，想放弃这次合并：`git merge --abort`（回到合并前的状态）。
...
Git 是开发者的基础工具。刚开始命令多、记不住很正常，先把本文的流程走通，之后每天用到的其实只有那十几条。遇到问题多读报错信息、多查资料，进步最快。
